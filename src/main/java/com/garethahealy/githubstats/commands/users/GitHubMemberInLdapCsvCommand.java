...
import com.garethahealy.githubstats.services.users.GitHubMemberInLdapCsvService;
...
@Dependent
@CommandLine.Command(name = "github-member-in-csv", mixinStandardHelpOptions = true, description = "Creates a single issue containing any users that are part of the GitHub Org but not in CSV")
public class GitHubMemberInLdapCsvCommand implements Runnable {
    ...
    @Inject
    GitHubMemberInLdapCsvService gitHubMemberInLdapCsvService;

    @Override
    public void run() {
        try {
            gitHubMemberInLdapCsvService.run(organization, orgRepo, dryRun, membersCsv, supplementaryCsv);
